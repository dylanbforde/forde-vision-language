...
def smooth_assignments(
    ...
    # Optimization: Use jax.vmap instead of a Python loop + jnp.stack for ~2x faster
    # JIT compile times and execution.
    def convolve_single_channel(channel):
        return convolve2d(channel, kernel, mode="same")

    # Vmap over the channels dimension (axis=-1)
    vmap_convolve = jax.vmap(convolve_single_channel, in_axes=-1, out_axes=-1)
    smoothed_padded_one_hot_grid = vmap_convolve(padded_one_hot_grid)
...
def smooth_assignments_3d(
    ...
    smoothed_one_hot = vmap_convolve(padded_grid)
