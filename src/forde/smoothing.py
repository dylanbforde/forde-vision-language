...
def smooth_assignments(
    ...
    # Bolt Optimization: Vectorize convolution over cluster channels using jax.vmap
    # This avoids tracing N separate convolutions and significantly speeds up compilation
    def _conv2d(channel, k):
        return convolve2d(channel, k, mode="same")

    out = jax.vmap(_conv2d, in_axes=(2, None))(padded_one_hot_grid, kernel)
    smoothed_padded_one_hot_grid = jnp.moveaxis(out, 0, -1)
...
def smooth_assignments_3d(
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        # Pad symmetrically where possible (only spatial dims, not clusters)
        padding = (
    ...
    # Bolt Optimization: Vectorize 3D convolution over cluster channels using jax.vmap
    def _conv3d(channel, k):
