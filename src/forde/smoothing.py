...
def smooth_assignments(
    ...
    # Optimization: Use jax.vmap to prevent massive XLA loop unrolling bloat
    # instead of jnp.stack with list comprehension over clusters.
    def conv_single(channel, k):
        return convolve2d(channel, k, mode="same")

    smoothed_padded_one_hot_grid = jax.vmap(
        conv_single, in_axes=(-1, None), out_axes=-1
...
def smooth_assignments_3d(
    ...
    # Optimization: Use jax.vmap to prevent massive XLA loop unrolling bloat
    # by vectorizing the convolution over the clusters axis, instead of a manual loop.
    def conv_single(channel, k):
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        ...
        smoothed_one_hot = jax.vmap(conv_single, in_axes=(-1, None), out_axes=-1)(
            padded_one_hot, kernel
        )
