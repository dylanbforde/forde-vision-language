...
def smooth_assignments(
    ...
    # Bolt Optimization: Use jax.vmap over the channel dimension instead of jnp.stack + Python loop.
    # This prevents XLA loop unrolling bloat, drastically reducing compilation time and memory usage.
    def convolve_fn(x, k):
        return convolve2d(x, k, mode="same")
...
def smooth_assignments_3d(
    ...
    # We must pad the grid to be at least kernel size in all spatial dims.
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        ...
        padding = (
            ...
            (pad_h // 2, pad_h - pad_h // 2),
            (pad_w // 2, pad_w - pad_w // 2),
            (0, 0),  # Do not pad the num_clusters dimension
        )
        # Bolt Optimization: Hoist jnp.pad out of the channel loop to pad the entire 4D array at once
    ...
    smoothed_padded_one_hot = jax.vmap(
        convolve_fn,
        in_axes=(-1, None),
        out_axes=-1,
    )(padded_one_hot_grid, kernel)
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        ...
        start_h = padding[1][0]
        start_w = padding[2][0]
        smoothed_one_hot = smoothed_padded_one_hot[
