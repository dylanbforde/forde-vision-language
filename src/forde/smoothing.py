...
def smooth_assignments(
    ...
    # Use vmap to apply convolution across the cluster dimension (avoid unrolled Python loop and jnp.stack)
    convolve_fn = jax.vmap(lambda x: convolve2d(x, kernel, mode="same"), in_axes=-1, out_axes=-1)
    smoothed_padded_one_hot_grid = convolve_fn(padded_one_hot_grid)
...
def smooth_assignments_3d(
    ...
    # Apply 3D convolution using vmap to process all clusters efficiently
    convolve_fn = jax.vmap(lambda x: convolve(x, kernel, mode="same"), in_axes=-1, out_axes=-1)
    smoothed_padded_one_hot = convolve_fn(padded_one_hot_grid)
