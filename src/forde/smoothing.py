...
def smooth_assignments(
    ...
    # Convolve on padded grid using vmap across the channel dimension
    vectorized_convolve = jax.vmap(
        lambda x, k: convolve2d(x, k, mode="same"), in_axes=(-1, None), out_axes=-1
    )
    smoothed_padded_one_hot_grid = vectorized_convolve(padded_one_hot_grid, kernel)
...
def smooth_assignments_3d(
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        # Pad symmetrically where possible (only spatial dims, not channel)
        padding = (
    ...
    # Apply 3D convolution per cluster channel using vmap to avoid loop unrolling overhead
    vectorized_convolve = jax.vmap(
        lambda x, k: convolve(x, k, mode="same"), in_axes=(-1, None), out_axes=-1
    )
    smoothed = vectorized_convolve(padded_one_hot_grid, kernel)
