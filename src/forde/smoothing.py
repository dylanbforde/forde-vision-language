...
def smooth_assignments(
    ...
    # Convolve on padded grid using vmap to avoid loop unrolling during JIT
    def convolve_channel(channel, k):
        return convolve2d(channel, k, mode="same")

    smoothed_padded_one_hot_grid = jax.vmap(
        convolve_channel, in_axes=(-1, None), out_axes=-1
    )(padded_one_hot_grid, kernel)
...
def smooth_assignments_3d(
    ...
    # We must pad the grid to be at least kernel size in all dims before convolving.
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        ...
        padded_one_hot_grid = jnp.pad(
            one_hot_grid, padding_config, "edge"
        )  # Use edge padding to extend values
    else:
        padded_one_hot_grid = one_hot_grid
        padding_config = ((0, 0), (0, 0), (0, 0), (0, 0))

    # Apply 3D convolution using vmap to avoid massive JIT unrolling overhead
    def convolve_channel(channel, k):
        return convolve(channel, k, mode="same")
