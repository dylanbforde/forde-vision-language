...
def smooth_assignments(
    ...
    # Convolve on padded grid using vmap to avoid loop unrolling
    # padded_one_hot_grid is (H, W, C). Transpose to (C, H, W) for vmap.
    channels = jnp.transpose(padded_one_hot_grid, (2, 0, 1))

    smoothed_channels = jax.vmap(lambda x: convolve2d(x, kernel, mode="same"))(channels)
...
def smooth_assignments_3d(
    ...
    # We must pad the tensor to be at least kernel size in all spatial dims.
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        ...
        # Use edge padding to extend values along spatial dimensions
        padded_one_hot = jnp.pad(one_hot_grid, padding, "edge")
    else:
        padded_one_hot = one_hot_grid
        padding = ((0, 0), (0, 0), (0, 0), (0, 0))

    # Apply 3D convolution using vmap over cluster channels
    # Transpose to (C, D, H, W) for vmap
    channels = jnp.transpose(padded_one_hot, (3, 0, 1, 2))

    # Convolve with mode='same'
    smoothed_channels = jax.vmap(lambda x: convolve(x, kernel, mode="same"))(channels)

    # Transpose back to (D, H, W, C)
    smoothed_one_hot_padded = jnp.transpose(smoothed_channels, (1, 2, 3, 0))
    ...
    if pad_d > 0 or pad_h > 0 or pad_w > 0:
        ...
        start_h = padding[1][0]
        start_w = padding[2][0]
        smoothed_one_hot = smoothed_one_hot_padded[
