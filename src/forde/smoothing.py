...
def smooth_assignments(
    ...
    smoothed_padded_one_hot_grid = convolver(padded_one_hot_grid, kernel)
...
def smooth_assignments_3d(
    ...
    # Apply 3D convolution using vmap over the last axis
    convolver = jax.vmap(
        lambda x, k: convolve(x, k, mode="same"), in_axes=(-1, None), out_axes=-1
    )
    smoothed_padded = convolver(padded_grid, kernel)
