...
def smooth_assignments(
    ...
    convolve_channels = jax.vmap(
        lambda channel: convolve2d(channel, kernel, mode="same"),
        in_axes=-1,
        out_axes=-1,
    )
    smoothed_padded_one_hot_grid = convolve_channels(padded_one_hot_grid)
...
def smooth_assignments_3d(
    ...
    # JAX convolve requires one input to be at least the kernel size in every
    # spatial dimension. Pad once before vmapping over cluster channels.
    ...
    convolve_channels = jax.vmap(
        lambda channel: convolve(channel, kernel, mode="same"),
        in_axes=-1,
        out_axes=-1,
    )
    smoothed_one_hot = convolve_channels(padded_one_hot_grid)
