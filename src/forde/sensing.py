...
def calculate_neuron_stats(activations, gradients):
    ...
    # Using explicit vectorization via `axis=0` instead of `jax.vmap` + transpose to
    # significantly reduce JIT compilation overhead and avoid large memory pre-allocations.
