...
def hoyer_sparsity(x, axis=-1):
    ...
    Args:
        x: A JAX array (vector or matrix).
    ...
    l2_norm = jnp.linalg.norm(x, axis=axis)
...
def calculate_neuron_stats(activations, gradients):
    ...
    # ⚡ Bolt Optimization: Calculate Hoyer sparsity directly using axis=0 to avoid
    # the substantial memory pre-allocation overhead and slowdown caused by jax.vmap
    # and matrix transpositions.
    ...
    # ⚡ Bolt Optimization: Use explicit axis=0 reduction instead of vmap(hoyer_sparsity)(gradients.T)
    ...
    neuron_stats = jnp.stack(
        [grad_gini, grad_gdp, act_gini, act_gdp, act_variance], axis=-1
    )
...
if __name__ == "__main__":
    ...
    print(
        f"\nSparse Activations Gini (first neuron): {sparse_stats[0, 2]}"
    )  # act_gini is 3rd stat
