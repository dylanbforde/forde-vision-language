...
def hoyer_sparsity(x, axis=-1):
    ...
    Args:
        x: A JAX array (vector or matrix). Sparsity is calculated along the given axis.
        axis: The axis along which to calculate sparsity. Defaults to -1 (last dimension).
...
def calculate_neuron_stats(activations, gradients):
    ...
    act_gini = hoyer_sparsity(
        activations, axis=0
    )  # Apply Hoyer's per neuron across batch
