...
def hoyer_sparsity(x, axis=-1):
    ...
    Args:
        x: A JAX array (vector or matrix). Sparsity is calculated along the given axis (default: -1).
        axis: The axis along which to calculate sparsity.
...
def calculate_neuron_stats(activations, gradients):
    ...
    act_gini = hoyer_sparsity(
        activations, axis=0
    )  # Explicit vectorization instead of vmap(transpose)
