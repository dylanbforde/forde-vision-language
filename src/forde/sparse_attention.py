...
class CompressedGlobalAttention(nn.Module):
    ...
    def __call__(
        ...
        # Reshape and pool
        pooled = global_tokens.reshape(batch_size, num_pools, self.compression_ratio, d_model)
...
class TopKSelection(nn.Module):
    ...
    def __call__(
        ...
        causal_bias = jnp.triu(jnp.full((seq_len, seq_len), -1e9), k=1)
        ...
        k_proj = nn.Dense(self.num_heads * self.head_dim, name="k_proj")(selected_tokens)
        k_proj = k_proj.reshape(batch_size, k, self.num_heads, self.head_dim)
        k_proj = k_proj.transpose(0, 2, 1, 3)
        
        v_proj = nn.Dense(self.num_heads * self.head_dim, name="v_proj")(selected_tokens)
...
class NativeSparseAttention(nn.Module):
    ...
    def __call__(
        ...
        if self.use_compressed:
            ...
            effective_start = jnp.maximum(local_window_start, self.compression_ratio + 1)
    ...
    def _compressed_global_attention(
        ...
        global_tokens = x[batch_idx, pool_indices[None, :], :]  # (batch, truncated_len, d_model)
        ...
        k = nn.Dense(self.num_heads * self.head_dim, name="compressed_k_proj")(compressed)
        k = k.reshape(batch_size, num_pools, self.num_heads, self.head_dim)
        k = k.transpose(0, 2, 1, 3)
        
        v = nn.Dense(self.num_heads * self.head_dim, name="compressed_v_proj")(compressed)
        v = v.reshape(batch_size, num_pools, self.num_heads, self.head_dim)
        v = v.transpose(0, 2, 1, 3)
    ...
    def _top_k_attention(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        k_proj = nn.Dense(self.num_heads * self.head_dim, name="topk_k_proj")(selected_tokens)
        k_proj = k_proj.reshape(batch_size, k, self.num_heads, self.head_dim)
        k_proj = k_proj.transpose(0, 2, 1, 3)
        
        v_proj = nn.Dense(self.num_heads * self.head_dim, name="topk_v_proj")(selected_tokens)
        v_proj = v_proj.reshape(batch_size, k, self.num_heads, self.head_dim)
        v_proj = v_proj.transpose(0, 2, 1, 3)
