...
class NativeSparseAttention(nn.Module):
    ...
    def _compressed_global_attention(
        ...
        # Bolt Optimization: Use slicing instead of gather when possible
        # for faster execution and cleaner graph.
        # Note: We need a fallback for safety if truncated_len > seq_len
        # (though logically it shouldn't be with the current formulas)
        if truncated_len <= seq_len:
            global_tokens = x[:, :truncated_len, :]
        else:
            pool_indices = jnp.arange(truncated_len) % seq_len
            batch_idx = jnp.arange(batch_size)[:, None]
            global_tokens = x[batch_idx, pool_indices[None, :], :]
