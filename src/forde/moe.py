...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over each expert individually to avoid creating a massive
        intermediate tensor of all expert outputs at once, which causes
        compilation bloat and OOM errors during large scale training.
        ...
        # Iterate over each expert individually to avoid large intermediate stacks
        for i, expert in enumerate(experts):
            # Create mask for tokens routed to this expert
            # expert_mask: (batch, seq, top_k)
            expert_mask = top_k_indices == i

            # token_mask: (batch, seq)
            token_mask = jnp.any(expert_mask, axis=-1)

            # Sum probabilities across any top-k routing slots that selected this expert
            ...
            # Accumulate using boolean masking
            output = output + jnp.where(
                token_mask[..., None], expert_weights * expert_out, 0.0
            )
