...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts to accumulate outputs, avoiding the massive memory
        overhead of eager computation and jnp.stack across all experts.
        ...
        # Iterate over each expert to compute and accumulate its contribution
        for i, expert in enumerate(experts):
            # Evaluate this expert on all tokens
            # (In a fully optimized capacity-based MoE, we'd only evaluate on selected tokens)
            expert_out = expert(x)

            # Find which tokens routed to this expert (across all top-k choices)
            # Shape: (batch, seq, top_k)
            expert_mask = top_k_indices == i

            # Sum the probabilities for this expert across top_k
            # Usually a token only picks an expert once, but this is robust
            # Shape: (batch, seq, 1)
            expert_weights = jnp.where(expert_mask, top_k_probs, 0.0).sum(
