...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        for i, expert in enumerate(experts):
            ...
            # Combine the weights over top_k for this expert
            weight_acc = jnp.zeros(x.shape[:2], dtype=jnp.float32)
            for k in range(self.top_k):
                is_selected = top_k_indices[..., k] == i
                weight_acc += jnp.where(is_selected, top_k_probs[..., k], 0.0)

            # Multiply and add to output
            output = output + jnp.where(
                weight_acc[..., None] > 0, expert_out * weight_acc[..., None], 0.0
            )
