...
class MoEStatefulLayer(nn.Module):
    ...
    def __call__(self, x) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        # Optimize: Use argmax and bincount instead of boolean masks to avoid large tensor allocations
        top1_indices = jnp.argmax(router_probs, axis=-1) # (batch_size, seq_len)
        flat_top1_indices = top1_indices.reshape(-1)
        flat_router_probs_max = router_probs.max(axis=-1).reshape(-1)

        top1_confidence_sum = jnp.bincount(flat_top1_indices, weights=flat_router_probs_max, length=self.num_experts)
        top1_count = jnp.bincount(flat_top1_indices, length=self.num_experts).astype(jnp.float32)
