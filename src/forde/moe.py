...
class MoEStatefulLayer(nn.Module):
    ...
    def __call__(self, x) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        # Optimize top1 selection by avoiding large boolean masks and redundant sums
        # Note: argmax handles ties by picking the first, avoiding double-counting
        top1_indices = router_probs.argmax(axis=-1).reshape(-1)
        top1_probs = router_probs.max(axis=-1).reshape(-1)

        top1_confidence_sum = jnp.bincount(
            top1_indices, weights=top1_probs, length=self.num_experts
        )
        top1_count = jnp.bincount(
            top1_indices, length=self.num_experts
        ).astype(jnp.float32)
