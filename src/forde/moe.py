...
class MoELayer(nn.Module):
    ...
    def _top_k_gating(self, router_logits: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
        """
    ...
    def _compute_expert_outputs(
        ...
        for k in range(self.top_k):
            ...
            selected_output = all_expert_outputs[expert_idx, batch_indices, seq_indices, :]
    ...
    def _load_balancing_loss(
        ...
        one_hot = jax.nn.one_hot(top_k_indices, num_experts)  # (batch, seq, top_k, num_experts)
...
class MoEStatefulLayer(nn.Module):
    ...
    def __call__(self, x) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        # This tracks which experts are being used and how often
        router = MoERouter(num_experts=self.num_experts)
