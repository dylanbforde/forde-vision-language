...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts individually to avoid allocating a massive
        intermediate tensor, preventing XLA compilation OOMs.
        ...
        # Process experts one by one to avoid large intermediate allocations
        for i, expert in enumerate(experts):
            # Calculate output only for this expert
            expert_out = expert(x)

            # Find where this expert was selected in the top-k
            # is_selected: (batch, seq, top_k)
            is_selected = top_k_indices == i

            # Get the probability weights for this expert if selected, sum over top_k dimension
            # expert_weights: (batch, seq, 1)
            expert_weights = jnp.sum(is_selected * top_k_probs, axis=-1, keepdims=True)

            # Accumulate using where to avoid useless computations where weight is 0
            output = jnp.where(
