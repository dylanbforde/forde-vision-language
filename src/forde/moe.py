...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts and accumulates the results to dramatically
        improve XLA compilation times and reduce memory overhead compared
        to eager jnp.stack computation.
        ...
        # Iterating over experts and accumulating avoids eagerly computing
        # and stacking all expert outputs into a massive intermediate tensor
        for i, expert in enumerate(experts):
            expert_out = expert(x)

            # Create a mask for where this expert was selected
            mask = (top_k_indices == i)

            # Get weights for this expert where selected, 0.0 otherwise
            weights = jnp.where(mask, top_k_probs, 0.0)

            # Sum weights along top_k dimension and add feature dimension
            expert_weight = jnp.sum(weights, axis=-1, keepdims=True)
