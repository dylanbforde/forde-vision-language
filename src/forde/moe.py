...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Optimized implementation: Iterates over experts individually and uses
        boolean masking to accumulate outputs, avoiding eager computation and
        stacking of all expert outputs into a large intermediate tensor.
        ...
        # Iterate over experts individually to prevent massive XLA memory allocation
        for i, expert in enumerate(experts):
            # Determine if this expert was selected for any top-k slot (batch, seq)
            is_selected = jnp.any(top_k_indices == i, axis=-1)

            # Compute expert output for all tokens
            expert_out = expert(x)

            # Extract weights for the selected expert
            # top_k_probs: (batch, seq, top_k), top_k_indices: (batch, seq, top_k)
            # We want to sum the probabilities where the expert was selected
            expert_weight = jnp.sum(
                top_k_probs * (top_k_indices == i), axis=-1, keepdims=True
            )

            # Accumulate output only where the expert is selected using masking
            output = output + jnp.where(
                jnp.expand_dims(is_selected, axis=-1),
                expert_out * expert_weight,
                jnp.zeros_like(expert_out),
            )
