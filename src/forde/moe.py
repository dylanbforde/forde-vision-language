...
class MoELayer(nn.Module):
    ...
    def _top_k_gating(
        ...
        """
        # Get top-k indices and values (logits) directly using lax.top_k
        # This is more efficient than argsort + gather (O(N) vs O(N log N))
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts and accumulates the results instead of eagerly computing
        and stacking all expert outputs into a massive intermediate tensor. This
        drastically improves XLA compilation times and reduces memory overhead.
        ...
        # Accumulate contributions from each expert sequentially
        for i, expert in enumerate(experts):
            # Evaluate expert on all tokens (can be further optimized if tokens are routed)
            expert_out = expert(x)

            # Find which tokens routed to this expert in any of the top-k positions
            expert_selected = (top_k_indices == i)  # (batch, seq, top_k)

            # Sum routing probabilities where this expert was selected
            # Shape: (batch, seq, 1)
            expert_weights = jnp.sum(top_k_probs * expert_selected, axis=-1, keepdims=True)

            # Only accumulate if the expert was selected at least once to avoid unnecessary math
            # Element-wise multiplication with routing weights accumulates only selected outputs
            # Using jnp.where explicitly handles condition to skip unused computations where possible
            output = output + jnp.where(
                jnp.any(expert_selected, axis=-1, keepdims=True),
    ...
    def _load_balancing_loss(
        ...
        # Optimized: Use bincount instead of one-hot + sum to avoid large tensor allocation
