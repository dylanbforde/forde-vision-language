...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Bolt Optimization: Iterating over experts and accumulating outputs via boolean masking (jnp.where)
        instead of computing and stacking all expert outputs at once. This drastically reduces memory
        allocations during XLA compilation and lowers total execution time, preventing silent OOMs.
        ...
        # Iterate over experts individually rather than stacking all outputs
        for i, expert in enumerate(experts):
            expert_out = expert(x)

            # For each top-k position, if this expert was selected, accumulate its output
            for k in range(self.top_k):
                is_selected = (top_k_indices[..., k] == i)[..., None]
                weight = top_k_probs[..., k : k + 1]

                # Accumulate using where to maintain stable XLA graph
                output = output + jnp.where(is_selected, expert_out * weight, 0.0)
