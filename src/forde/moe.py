...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        # Bolt Optimization: Avoid eagerly stacking all expert outputs into a massive
        # intermediate tensor which causes XLA compilation OOMs and bloat.
        # Instead, iterate over experts, compute outputs, and accumulate via masking.
        for i, expert in enumerate(experts):
            expert_out = expert(x)

            # Mask where this expert is selected in any of the top_k positions
            expert_mask = top_k_indices == i

            # Get the probability weights for this expert across all tokens
            expert_weights = expert_mask * top_k_probs

            # Sum weights across the top_k dimension (in case an expert is selected multiple times,
            # though usually it's selected at most once per token)
            total_expert_weight = jnp.sum(expert_weights, axis=-1, keepdims=True)

            # Identify tokens where this expert was selected
            is_selected = total_expert_weight > 0

            # Accumulate the weighted expert output for selected tokens
            output = jnp.where(
                is_selected, output + total_expert_weight * expert_out, output
            )
