...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        This uses an optimized approach that iterates over experts and accumulates
        outputs using boolean masking. This avoids memory bloat and silent OOMs
        associated with eagerly stacking all expert outputs (`jnp.stack`).
        ...
        for i, expert in enumerate(experts):
            # Compute expert output lazily to avoid peak memory spike
            expert_out = expert(x)

            # Create mask for tokens that selected this expert
            mask = top_k_indices == i

            # Combine weights from any top-k slot that selected this expert
