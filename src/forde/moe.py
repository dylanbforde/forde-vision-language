...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        This is a memory-efficient implementation that iterates over experts and
        accumulates outputs via boolean masking instead of computing all outputs
        in parallel and stacking them, avoiding massive intermediate tensors.
        ...
        # For each expert, evaluate only if it was selected somewhere in the batch
        # Accumulate outputs using a boolean mask to avoid gathering from a massive stacked tensor.
        for i, expert in enumerate(experts):
            # Create a mask indicating where this expert was selected
            # Shape: (batch, seq, top_k)
            expert_mask = top_k_indices == i

            # Sum the probabilities for this expert (in case chosen multiple times, though rare)
            # Shape: (batch, seq, 1)
            ...
            # Accumulate using where to ensure no unwanted values flow if weight is 0
            weighted_output = expert_weights * expert_output
            output = jnp.where(expert_weights > 0, output + weighted_output, output)
