...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts individually, calculates outputs, and accumulates
        using boolean masking to avoid eager memory allocation and compilation OOMs.
        ...
        # Iterate over experts and accumulate outputs via boolean masking
        for expert_id, expert in enumerate(experts):
            # Compute expert output only once
            expert_out = expert(x)

            # Create mask for where this expert was selected
            expert_mask = top_k_indices == expert_id

            # Get weights for this expert where it was selected, else 0
            expert_weights = jnp.sum(
                jnp.where(expert_mask, top_k_probs, 0.0), axis=-1, keepdims=True
            )

            # Accumulate output: only multiply if weight > 0
            output = output + jnp.where(
                expert_weights > 0, expert_weights * expert_out, 0.0
            )
