...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts and accumulates the results to avoid massive intermediate
        tensor allocations (which cause silent OOMs on large XLA compilations).
        ...
        # Iterate over all experts to accumulate output, which avoids massive
        # OOM-inducing jnp.stack arrays in XLA compilation.
        for i, expert in enumerate(experts):
            # Calculate total weight assigned to this expert across the top-k selections
