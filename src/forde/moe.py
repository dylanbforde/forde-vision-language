...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        # all expert outputs into a massive intermediate tensor using jnp.stack.
        # This reduces memory allocations during compilation and avoids silent OOMs.
        for i, expert in enumerate(experts):
            # Compute this expert's output
            expert_out = expert(x)

            # Create a mask for where this expert was selected in the top-k
            # Shape: (batch, seq, top_k)
            expert_mask = top_k_indices == i

            # Sum the probabilities where this expert was selected (usually at most 1 place)
            # Shape: (batch, seq, 1)
