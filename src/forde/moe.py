...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        """
        # Compute output from all experts and combine using boolean masking
        # This avoids eagerly stacking all expert outputs into a massive tensor
        # which can cause OOMs during XLA compilation and is much faster to compile.

        # Initialize output accumulator
        output = jnp.zeros_like(x)

        # Iterate over experts rather than top_k indices
        for i, expert in enumerate(experts):
            # Compute this expert's output
            expert_out = expert(x)  # (batch, seq, d_model)

            # Check where this expert was selected in the top_k
            for k in range(self.top_k):
                # Mask: 1 if expert i is selected at position k, 0 otherwise
                # Shape: (batch, seq, 1)
                ...
                # Accumulate weighted output where selected
                output = output + jnp.where(is_selected, expert_out * weights, 0.0)
