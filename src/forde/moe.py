...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        This avoids jnp.stack([expert(x)...]) which causes massive XLA compilation
        bloat and OOMs. Instead, we accumulate outputs per expert dynamically.
        ...
        # Iterate over experts and accumulate their weighted contributions
        for i, expert in enumerate(experts):
            expert_out = expert(x)

            # Mask to find which tokens routed to this expert
            expert_mask = top_k_indices == i

            # Sum probabilities across the top-k dimension for this expert
            expert_weights = jnp.sum(
                jnp.where(expert_mask, top_k_probs, 0.0), axis=-1, keepdims=True
            )

            # Weighted sum for this expert
