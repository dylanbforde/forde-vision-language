...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts to accumulate outputs. Avoids eagerly stacking
        all expert outputs into a massive memory-bloating intermediate tensor,
        significantly reducing JAX/XLA compilation times and memory usage.
        ...
        # Compute combined routing weights for all experts to avoid massive intermediate stack
        for i, expert in enumerate(experts):
            # Find tokens assigned to this expert across all top_k positions
            expert_weight = jnp.sum(
                jnp.where(top_k_indices == i, top_k_probs, 0.0), axis=-1
            )

            # Compute expert output (memory overhead is just 1 expert vs all)
            expert_out = expert(x)

            # Accumulate weighted expert output
            output = output + expert_weight[..., None] * expert_out
