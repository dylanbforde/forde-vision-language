...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Iterates over experts and accumulates the result to avoid large intermediate
        tensor allocations and XLA compilation overhead.
        ...
        # Process each expert one by one and accumulate the result.
        # This prevents the creation of a massive (num_experts, batch, seq, d_model) tensor.
        for i, expert in enumerate(experts):
            # Find tokens where this expert is among the top_k
            prob_mask = jnp.where(top_k_indices == i, top_k_probs, 0.0)
            expert_weights = jnp.sum(prob_mask, axis=-1)  # (batch, seq)

            expert_out = expert(x)

            # Add weighted expert output
            output = output + jnp.expand_dims(expert_weights, axis=-1) * expert_out
