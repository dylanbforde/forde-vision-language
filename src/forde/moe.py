...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        This implementation iterates over experts individually to avoid allocating
        a massive intermediate tensor via jnp.stack, which causes OOM on large
        XLA compilations.
        ...
        # Iterate over experts to avoid computing and stacking all outputs eagerly
        for i, expert in enumerate(experts):
            # Calculate output for this expert
            expert_output = expert(x)

            # Accumulate weight for this expert across all k
            expert_weight = jnp.zeros(x.shape[:-1], dtype=x.dtype)
            for k in range(self.top_k):
                is_selected = top_k_indices[..., k] == i
                expert_weight = jnp.where(
                    is_selected, top_k_probs[..., k], expert_weight
                )

            # Add weighted expert output to total output
            output = output + expert_weight[..., None] * expert_output
