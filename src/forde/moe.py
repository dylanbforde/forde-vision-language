...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        # Iterate over experts and accumulate the results using element-wise multiplication
        # with routing weights to drastically improve XLA compilation times and reduce memory
        # overhead compared to eager stacking of all expert outputs.
        for i, expert in enumerate(experts):
            expert_out = expert(x)

            # Combine weights from all k where this expert was selected
            weight = jnp.zeros((batch_size, seq_len), dtype=x.dtype)
            for k in range(self.top_k):
                mask = top_k_indices[..., k] == i
                weight = weight + jnp.where(mask, top_k_probs[..., k], 0.0)

            output = output + weight[..., None] * expert_out
