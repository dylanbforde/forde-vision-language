...
class MoEStatefulLayer(nn.Module):
    ...
    def __call__(self, x) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        # Optimized: Use argmax + bincount instead of boolean mask + where + sum
        # This prevents large tensor allocations and gives significant speedup
        top1_indices = jnp.argmax(router_probs, axis=-1).reshape(-1)
        top1_probs = jnp.max(router_probs, axis=-1).reshape(-1)
