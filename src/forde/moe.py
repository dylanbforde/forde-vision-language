...
class MoELayer(nn.Module):
    ...
    def _compute_expert_outputs(
        ...
        Optimized implementation: Iterates through experts and accumulates outputs
        using boolean masks instead of stacking all outputs, reducing XLA
        ...
        for i in range(num_experts):
            ...
            # Extract routing weights for tokens that selected expert i
            expert_weights = jnp.where(expert_mask, top_k_probs, 0.0).sum(
                axis=-1, keepdims=True
            )

            # Accumulate output
            output = output + expert_weights * experts[i](x)
