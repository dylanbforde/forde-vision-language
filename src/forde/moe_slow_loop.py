...
def calculate_expert_stats(router_probs: jnp.ndarray) -> jnp.ndarray:
    ...
    confidence_sum = jnp.bincount(top1_indices, weights=flat_probs, length=num_experts)
    selection_count = jnp.bincount(top1_indices, length=num_experts)
    expert_selection_confidence = confidence_sum / (selection_count + eps)
