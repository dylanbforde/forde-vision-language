...
def calculate_expert_stats(router_probs: jnp.ndarray) -> jnp.ndarray:
    ...
    confidence_sum = jnp.bincount(top1_indices, weights=top1_probs, length=num_experts)
    count = jnp.bincount(top1_indices, length=num_experts).astype(jnp.float32)

    expert_selection_confidence = confidence_sum / (count + eps)
