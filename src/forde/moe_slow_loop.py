...
def calculate_expert_stats(
    ...
    expert_outputs: Optional[jnp.ndarray] = None
) -> jnp.ndarray:
    """
    ...
    eps = 1e-8
    entropy = -(router_probs * jnp.log(router_probs + eps)).sum(axis=-1).mean(axis=(0, 1))
    entropy_per_expert = jnp.full(num_experts, entropy)  # Same for all experts in basic version
    ...
    expert_selection_confidence = jnp.where(
        max_probs_mask,
        router_probs,
        0.0
    ).sum(axis=(0, 1)) / (max_probs_mask.sum(axis=(0, 1)) + eps)
...
def moe_slow_loop_step(
    ...
    try:
        ...
        smoothed_grid = smooth_assignments_3d(assignment_grid, kernel_size=3, num_clusters=3)
    ...
    if 'stats_buffer' in mutable_vars_unfrozen:
        mutable_vars_unfrozen['stats_buffer'] = jax.tree.map(
            reset_leaf, 
            mutable_vars_unfrozen['stats_buffer']
        )
...
if __name__ == '__main__':
    ...
    router_probs = jax.nn.softmax(
        jax.random.normal(key, (batch_size, seq_len, num_experts)),
        axis=-1
    )
