...
def moe_slow_loop_step(
    ...
    def update_router_bias(path, param):
        ...
        # Safely extract string keys from path
        path_str = []
        for p in path:
            if hasattr(p, "key"):
                path_str.append(str(p.key))
            else:
                path_str.append(str(p))

        if "router_linear" in path_str and "bias" in path_str:
    ...
    # Traverse and update using jax.tree_util.tree_map_with_path
    # This avoids intermediate flatten/unflatten allocations and works with FrozenDict
