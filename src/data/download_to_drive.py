...
try:
    ...
    def process_image(image_url):
        """Download and normalize one RGB image for Conceptual Captions shards."""
        import io
...
def mount_drive():
    ...
    try:
        ...
        print(
            "If running in Colab terminal, please mount Drive in a notebook cell first:"
        )
...
def download_and_save(output_dir, num_proc=4, max_samples=None, shard_size=5000):
    ...
    existing_shards = [
        ...
        if d.startswith("shard_") and os.path.isdir(os.path.join(output_dir, d))
    ]
    ...
    if shard_indices:
        ...
        print(
            f"Found {len(shard_indices)} existing shards. Resuming from shard {next_shard_idx} (skipping {samples_processed} samples)."
        )
    else:
        next_shard_idx = 0
        samples_processed = 0
    ...
    dataset = datasets.load_dataset(
        "conceptual_captions", streaming=True, split="train"
    )
        ...
        def process_batch(batch_size=100):
            with ThreadPoolExecutor(max_workers=num_proc) as executor:
                while True:
                    ...
                    futures = [
                        executor.submit(process_example, ex, tokenizer) for ex in chunk
                    ]
    ...
    try:
        for example in tqdm(gen, desc="Processing"):
            ...
            if len(current_shard_data) >= shard_size:
                ...
                shard_dataset = datasets.Dataset.from_list(
                    current_shard_data, features=features
                )

                with tempfile.TemporaryDirectory(dir=temp_base) as temp_shard_dir:
                    print(f"Saving shard {shard_counter} to temporary local storage")
        ...
        if current_shard_data:
            print("Final shard collected. Converting to Arrow format...")
            shard_dataset = datasets.Dataset.from_list(
                current_shard_data, features=features
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download and process dataset to Drive"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="forde_dataset",
        help="Directory name to save dataset",
    )
    parser.add_argument(
        "--num_proc", type=int, default=8, help="Number of worker threads"
    )
    parser.add_argument(
        "--max_samples",
        type=int,
    ...
    parser.add_argument(
        "--shard_size", type=int, default=1000, help="Number of samples per shard"
    )
